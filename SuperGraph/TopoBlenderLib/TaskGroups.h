...
struct TaskGroups{
	...
	static QVector< QList<Task*> > split( QList<Task*> list, Structure::Graph * graph )
	{
		...
		// Visited tasks or not
                QMap<Structure::Node*,bool> visited;
                foreach(Task* t, list) visited[t->node()] = false;

		// Tasks from nodes
                QMap<Structure::Node*,Task*> taskMap;
		...
		foreach(Task* t, list)
		{
			...
			while( !tasksToVisit.empty() )
			{
				...
				curSet.push_back( curTask );
                                ...
                                QVector<Structure::Node*> adjNodes = graph->adjNodes(curNode);
	...
	struct Graph
	{
		...
		Graph(QList<Task*> list, Structure::Graph * graph)
		{
			...
			if(list.size() == 2)
			{
                                Structure::Node * first = list.front()->node();
                                Structure::Node * last = list.back()->node();
