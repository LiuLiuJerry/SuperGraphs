...
/*Link &Link::operator= (const Link &other){
    ...
    int len = other.coord.size();
    for(int i = 0; i < len; i++){
        this->coord.push_back(other.coord[i]);
    }
    return *this;
}*/
...
{
    Node * n = n1->id == nodeID ? n1 : n2; //Jerry's change
    //Node * n = n1->id == nodeID ? n2 : n1;
    assert(n->id == nodeID); //Jerry's change
